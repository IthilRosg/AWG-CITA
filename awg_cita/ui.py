    ...
    <main id="app" class="app" aria-busy="true">
      ...
      <section id="settings-view" class="view" data-view-panel="settings" aria-labelledby="settings-heading" hidden>
        ...
        <div class="section-heading server-settings-heading"><div>
          ...
          <p id="server-settings-note" class="section-subtitle">Изменение порта, адреса или обфускации прерывает подключения выбранного интерфейса. После изменения скачайте конфиги клиентов заново.</p>
        </div></div>
        <div class="profile-settings-grid server-settings-grid">
          <div class="panel server-settings-card" data-server-profile="awg3"><strong>AWG 3.1</strong><div class="server-settings-content" role="status">Загрузка…</div><form class="server-endpoint-form" data-endpoint-profile="awg3"><label>Endpoint host<input name="endpoint" autocomplete="off" required maxlength="253"></label><div class="profile-settings-actions"><span data-endpoint-status="awg3" role="status"></span><button class="button" type="submit">Сохранить endpoint</button></div></form><form class="server-endpoint-form" data-port-profile="awg3"><label>UDP порт<input name="listenPort" type="number" min="1" max="65535" required></label><div class="profile-settings-actions"><span data-port-status="awg3" role="status"></span><button class="button" type="submit">Перенести порт</button></div></form></div>
