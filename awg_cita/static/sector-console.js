(() => {
  ...
  async function saveServerEndpoint(form) {
    const profile = form.dataset.endpointProfile;
    const endpoint = form.elements.namedItem('endpoint').value.trim().toLowerCase();
    const button = form.querySelector('button');
    const status = document.querySelector(`[data-endpoint-status="${profile}"]`);
    if (!realCanary || !/^[0-9a-f]{64}$/.test(form.dataset.revision || '')) return;
    button.disabled = true;
    status.textContent = state.locale === 'ru' ? 'Применяю…' : 'Applying…';
    try {
      const value = await adapter.updateServerEndpoint(profile, form.dataset.revision, endpoint);
      if (value?.schema_version !== 1 || value.profile !== profile || value.endpoint !== endpoint) throw new Error('invalid_server_settings');
      status.textContent = state.locale === 'ru' ? 'Сохранено. Скачайте конфиги клиентов заново.' : 'Saved. Download client configs again.';
